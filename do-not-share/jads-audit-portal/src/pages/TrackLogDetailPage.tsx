...
export function TrackLogDetailPage() {
  ...
  useEffect(() => {
    ...
    ;(async () => {
      ...
      try {
        const { data } = await auditAxios(token).get('/track-logs/' + id)
  ...
  useEffect(() => {
    ...
    if (pathPoints.length > 0) {
      ...
      // Start marker
      L.circleMarker(latlngs[0], { radius: 8, fillColor: T.green, color: T.bg, fillOpacity: 1, weight: 2 })
      ...
      if (log.takeoffJson) {
        try {
          ...
          if (tk.lat && tk.lon) {
            L.circleMarker([tk.lat, tk.lon], { radius: 8, fillColor: T.green, color: T.bg, fillOpacity: 1, weight: 2 })
  ...
  return (
    <div style={{ padding: '1.5rem', maxWidth: '900px', fontFamily: "'JetBrains Mono', monospace", color: T.text }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '0.6rem', fontSize: '0.75rem' }}>
        ...
        <Detail label="Breach Count" value={String(log.breachCount)} color={log.breachCount > 0 ? T.red : T.green} />
