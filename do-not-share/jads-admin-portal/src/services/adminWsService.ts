// T10 — Admin Portal WebSocket client (subscribes to ALL missions)
...
export class AdminWsService {
  ...
  connect(wsUrl: string, token: string) {
    ...
    const url = `${wsUrl}/ws/missions?token=${token}&subscribe=ALL`
    ...
    this.ws.onclose = () => {
      ...
      this.reconnectTimer = setTimeout(
        () => this.connect(this._wsUrl, this._token),
        this.retryDelay,
      )
  ...
  private handleMessage(msg: { type: string; data: any }) {
    switch (msg.type) {
      case 'TELEMETRY_POINT':    this._onTelemetry(msg.data); break
      case 'GEOFENCE_VIOLATION': this._onViolation(msg.data); break
      case 'BATTERY_CRITICAL':   this._onBatteryCritical(msg.data); break
    }
  }
