...
- Modify audit log entries (PostgreSQL triggers block UPDATE/DELETE at database engine level)
...
| Runtime integrity | SHA-256 baseline of critical files, periodic re-check | `RuntimeIntegrityService` |
| Audit immutability | PostgreSQL BEFORE UPDATE/DELETE triggers raise exception | DB migration triggers |
...
- Audit log append-only — PostgreSQL triggers block modification
...
- RuntimeIntegrityService detects file modifications at runtime
